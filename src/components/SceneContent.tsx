...
import { Group, Raycaster, Vector2, Mesh, MeshStandardMaterial, Object3D, Object3DEventMap } from 'three';
...
const SceneContent: React.FC<SceneContentProps> = ({ models, selectedModel, activeTool }) => {
  ...
  const handleObjectClick = (mesh: Object3D<Object3DEventMap>, uuid: string) => {
  ...
  const handleTransformChange = () => {
    if (selectedModel) {
      ...
      dispatch(updateModelTransform({ id: selectedModelId as string, position, rotation, scale }));
    }
  };
  ...
  return (
    <>
      ...
      {Object.values(models).map((model, index) => (
        <primitive object={model} key={index} onClick={() => handleObjectClick(model.children[0], model.uuid)}/>
      ))}
    </>
  );
};
