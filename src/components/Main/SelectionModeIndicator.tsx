...
const SelectionModeIndicator: React.FC = () => {
  ...
  const currentSubObjectType = useSelector((state: RootState) => state.ui.currentSubObjectType);
  const selectionMode = useSelector((state: RootState) => state.ui.subObjectSelectionMode);
  ...
  return (
    <div style={{
      ...
      <div><strong>Mode:</strong> {currentSubObjectType}</div>
      <div><strong>Selection:</strong> {selectionMode}</div>
